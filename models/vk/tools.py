...
def get_token_and_user_id(login, password):
    vk = VKAuth(['ads,offline,groups'], '5776857', '5.103', email=login, pswd=password)
    vk.auth()
    token = vk.get_token()
...
class CPMCalculator:
    ...
    def __init__(self, target_rate=0.04, stop_rate=0.03, target_cost=1., stop_cost=2., cpm_step=10., cpm_limit=120.):
        self.target_rate = target_rate
        self.stop_rate = stop_rate
        self.target_cost = target_cost
        self.stop_cost = stop_cost
        self.cpm_step = cpm_step
        self.cpm_limit = cpm_limit
        self.stopped_list = []
        ...
        for ad_id, current_rate in current_rates:
            ...
            if current_rate < self.target_rate:
                ...
                if current_rate < self.stop_rate:
                    if ad_id not in self.stopped_list:
            ...
            # Если текущая конверсия больше целевой - повышаем ставку
                ...
                if current_cpm + self.cpm_step <= self.cpm_limit:
                    cpm_dict[ad_id] = current_cpm + self.cpm_step
                else:
                    cpm_dict[ad_id] = self.cpm_limit

        self.stopped_list.extend(stop_ads)

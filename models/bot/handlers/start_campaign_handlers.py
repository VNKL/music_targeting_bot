...
def _sc_select_campaign_to_start(update, context):
    ...
    if _is_user_known(context, update):
        ...
        if text in list(campaigns.keys()):
            ...
            context.bot.send_message(chat_id=update.effective_chat.id,
                                     text=f'Автоматизировать кампанию полностью или просто запустить?',
                                     reply_markup=ReplyKeyboardMarkup([['Автоматизировать полностью'],
                                                                       ['Просто запустить']], one_time_keyboard=True))
            return 'automate_campaign_decision'
        else:
            context.bot.send_message(chat_id=update.effective_chat.id,
...
def _sc_automate_decision(update, context):
    logging.info(f'SC - {update.effective_user.username} trying to set automate decision')

    if _is_user_known(context, update):
        user = DB.users.find_one({'user_id': update.effective_user.id})
        text = update.message.text

        if text == 'Автоматизировать полностью':
            context.bot.send_message(chat_id=update.effective_chat.id,
            ...
            logging.info(f'SC - {update.effective_user.username} chose to automate campaign')
            return 'get_rates'

        elif text == 'Просто запустить':
            campaigns = get_campaigns_from_db(update)
            ...
            logging.info(f'SC - {update.effective_user.username} chose to only start campaign')
...
def _sc_get_rates(update, context):
    logging.info(f'SC - {update.effective_user.username} trying to set rates')
            ...
            try:
                ...
                context.bot.send_message(chat_id=update.effective_chat.id,
                                         text=f'Пришли через пробел целевую и максимальную стоиомсть клика на плей в '
                                              ...
                                              f'Например: 0.9 1.3')
                logging.info(f'SC - {update.effective_user.username} set rates')
                return 'get_costs'
...
def _sc_get_costs(update, context):
    logging.info(f'SC - {update.effective_user.username} trying to set costs')
    ...
    if _is_user_known(context, update):
        ...
        if len(text) != 2:
            ...
            try:
                ...
                logging.info(f'SC - {update.effective_user.username} set costs')
...
def _sc_get_cpm_step_and_limit(update, context):
    logging.info(f'SC - {update.effective_user.username} trying to set cpm step and cpm limit')
    ...
    if _is_user_known(context, update):
        ...
        if len(text) != 2:
            ...
            try:
                ...
                logging.info(f'SC - {update.effective_user.username} set cpm step and cpm limit')
...
def _sc_get_cpm_update_interval(update, context):
    ...
    if _is_user_known(context, update):
        ...
        try:
            ...
            context.bot.send_message(chat_id=update.effective_chat.id,
                                     text=_sc_preparation_summary(update),
...
def _sc_confirm_automate(update, context):
    logging.info(f'SC - {update.effective_user.username} trying to end dialog')
    ...
    if _is_user_known(context, update):
        ...
        if text == 'Нет, давай заново':
            ...
            campaign = campaigns[start_campaign_settings[user['user_id']]['campaign_name']]
            print(campaign)
            process = Process(target=fully_automate_campaign, args=(
                update,
                campaign,
...
def _sc_preparation_summary(update):
    user = DB.users.find_one({'user_id': update.effective_user.id})
...
start_campaign_handler = ConversationHandler(
    ...
    states={
        ...
        'automate_campaign_decision': [CommandHandler('reload', reload),
                                       MessageHandler(Filters.text, _sc_automate_decision)],
        'get_rates': [CommandHandler('reload', reload),
                      MessageHandler(Filters.text, _sc_get_rates)],
        'get_costs': [CommandHandler('reload', reload),
                      MessageHandler(Filters.text, _sc_get_costs)],
        'get_cpm_step_and_limit': [CommandHandler('reload', reload),
                                   MessageHandler(Filters.text, _sc_get_cpm_step_and_limit)],
        'get_cpm_update_interval': [CommandHandler('reload', reload),
                                    MessageHandler(Filters.text, _sc_get_cpm_update_interval)],
        'confirm_automate': [CommandHandler('reload', reload),
                             MessageHandler(Filters.text, _sc_confirm_automate)]
    },
